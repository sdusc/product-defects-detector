...
class PDD_DATASET(Dataset):
    """
    Class to load DataSets
    Root is the path of subsets
    """

    def __init__(self, root):
        self.classes = ["Good", "Anomaly"] if NEG_CLASS == 1 else ["Anomaly", "Good"]
        self.img_transform = transforms.Compose(
            [transforms.Resize(INPUT_IMG_SIZE), transforms.ToTensor()]  # transform image size to 224*224
        )
